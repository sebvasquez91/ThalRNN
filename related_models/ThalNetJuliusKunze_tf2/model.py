...
class Classifier:
    ...
    @lazy_property
    def logits(self):
        return tf.keras.layers.Dense(self.run, num_outputs=int(self.target.shape[1]),
                                                 activation_fn=None)
    ...
    def cross_entropy(self):
        return tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=tf.stop_gradient(self.target)))

    @define_scope
    def accuracy(self):
        correct = tf.equal(tf.argmax(input=self.logits, axis=1), tf.argmax(input=self.target, axis=1))
        return tf.reduce_mean(input_tensor=tf.cast(correct, dtype=tf.float32))

    @define_scope
    def optimize(self):
        return tf.compat.v1.train.AdamOptimizer(learning_rate=1e-3).minimize(self.cross_entropy)

    @define_scope('weights')
    def weights_summary(self):
        variables_except_from_optimizer = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='^(?!.*optimize).*$')

        return tf.compat.v1.summary.merge([tf.compat.v1.summary.histogram(v.name, v) for v in variables_except_from_optimizer])

    def summary(self):
        test_cross_entropy_summary = tf.compat.v1.summary.scalar(f'cross_entropy', self.cross_entropy)
        test_accuracy_summary = tf.compat.v1.summary.scalar(f'accuracy', self.accuracy)
        return tf.compat.v1.summary.merge([test_cross_entropy_summary, test_accuracy_summary])
    ...
    def num_parameters(self):
        return np.sum([np.prod(v.shape) for v in tf.compat.v1.trainable_variables()])
...
class MLPClassifier(Classifier):
    ...
    def run(self):
        ...
        for _ in range(self.num_layers):
            lastoutput = tf.keras.layers.Dense(lastoutput,num_outputs=self.num_hidden,activation_fn=tf.nn.relu)
        return lastoutput

class SequenceClassifier(Classifier):
    def __init__(self, data, target, dropout,
                 get_rnn_cell: Callable[[], tf.compat.v1.nn.rnn_cell.RNNCell],num_rows,row_size):
    ...
    @lazy_property
    def run(self):
        rnn_cell_with_dropout = tf.compat.v1.nn.rnn_cell.DropoutWrapper(self.get_rnn_cell(), output_keep_prob=1 - self.dropout)
        output, last_state = tf.compat.v1.nn.dynamic_rnn(rnn_cell_with_dropout, self.data, dtype=tf.float32)
        output = tf.transpose(a=output, perm=[1, 0, 2])
        last_output = tf.nn.embedding_lookup(params=output, ids=int(output.shape[0])-1)
        return last_output
...
def GRUCell(num_hidden: int, num_layers=4):
    return tf.compat.v1.nn.rnn_cell.MultiRNNCell([tf.compat.v1.nn.rnn_cell.GRUCell(num_hidden) for _ in range(num_layers)])
...
class FfGruModule:
    ...
    def __call__(self, inputs, center_state, module_state):
        ...
        with tf.compat.v1.variable_scope(self.name):
            reading_weights = tf.compat.v1.get_variable('reading_weights',shape=[self.center_size,self.context_input_size],initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
            ...
            inputs = tf.keras.layers.Dense(inputs)

            gru = tf.compat.v1.nn.rnn_cell.GRUCell(self.num_gru_units)
...
class ThalNetCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    def __init__(self,
                 input_size: int,
                 output_size: int,
                 context_input_size: int,
